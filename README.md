...
- Does not detect `Path::try_exists()` (stabilized in Rust 1.63): the `?` operator in the condition desugars to a `Match` node, so the condition is not seen as a simple `.exists()` call.
...
The biggest thing was getting hands-on with Clippy's tooling — how lints are scaffolded, how the HIR is structured, and how `cargo dev` automates a lot of the registration boilerplate that would otherwise be easy to get wrong. Working that close to the compiler internals was new territory for me coming from application-level Rust.

### Challenges Overcome

Thinking through edge cases was harder than understanding the bug itself. The TOCTOU race made sense immediately, but mapping out all the ways it could appear in real code — the `?` operator desugaring, compound `&&` conditions, the stored-bool variant — took more iteration than expected. Each edge case required digging into how Rust represents that pattern in the HIR, which wasn't always obvious upfront.

### What I'd Do Differently Next Time

I'd probably look for a different issue or project for a second contribution — not because this one went badly, but because starting fresh on something new would push me to apply what I learned here rather than staying in familiar territory.
